...
MQTTLog.prototype.invoke = function MQTTLog_invoke(context, next) {   
    ...
    if(context["server.IsLocal"])
    {
        ...
        console.log("[MQTT-LOG] REQUEST IN " + _requestLog(context))
        return next();
    }
    else {
        console.log("[MQTT-LOG] RESPONSE IN " + _responseLog(context));
        return next();
    }
}
...
MQTTLog.prototype._write = function _MQTTLog_write(context, nextStream, chunk, encoding, callback) {
    ...
    if(context["server.IsLocal"])
        {
            console.log("[MQTT-LOG] REQUEST OUT " + _requestLog(context));
        }
        else {
            console.log("[MQTT-LOG] RESPONSE OUT " + _responseLog(context));
      }
...
function _responseLog(context)
{
    ...
    var body = response["iopa.Body"].toString();
