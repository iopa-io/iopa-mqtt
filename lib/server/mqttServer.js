...
var IopaServer = require('./iopqServer.js');
...
function MQTTServer(options, appFunc) {
  if (!(this instanceof MQTTServer))
    ...
    *   this.serverMessagePipeline
    *   this.clientMessagePipeline
    */
   IopaServer.call(options, appFunc);
        
   // INIT TCP SERVER
  this._tcp = new TcpServer(options, this.serverMessagePipeline, this.clientMessagePipeline);
}
...
MQTTServer.prototype._serverChannelPipelineSetup = function (serverChannelApp) {
  ...
  serverChannelApp.use(MQTTAutoAck);
};
 ...
 * @public
 */
MQTTServer.prototype.createChannel = function mqttServer_createChannel(urlStr) {
  var that = this;
  return this._tcp.connect(urlStr).then(function(client){
    var p =  that._clientChannelPipeline(client);
